...
class WindowAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.shifted:
            dots[:, :, -nw_w:] += self.upper_lower_mask     # 最后一行窗口加mask
            dots[:, :, nw_w - 1::nw_w] += self.left_right_mask     # 最后一列窗口加mask
...
def swin_t(patch_size=2, hidden_dim=96, layers=(2, 2, 6, 2), heads=(3, 6, 12, 24), **kwargs):
    return SwinTransformer(patch_size=patch_size,hidden_dim=hidden_dim, layers=layers, heads=heads, **kwargs)


def swin_s(patch_size=2, hidden_dim=96, layers=(2, 2, 18, 2), heads=(3, 6, 12, 24), **kwargs):
    return SwinTransformer(patch_size=patch_size,hidden_dim=hidden_dim, layers=layers, heads=heads, **kwargs)


def swin_b(patch_size=2, hidden_dim=128, layers=(2, 2, 18, 2), heads=(4, 8, 16, 32), **kwargs):
    return SwinTransformer(patch_size=patch_size, hidden_dim=hidden_dim, layers=layers, heads=heads, **kwargs)


def swin_l(patch_size=2, hidden_dim=192, layers=(2, 2, 18, 2), heads=(6, 12, 24, 48), **kwargs):
    return SwinTransformer(patch_size=patch_size, hidden_dim=hidden_dim, layers=layers, heads=heads, **kwargs)
...
if __name__ == '__main__':
    ...
    dummy_x = torch.randn(1, 3, 896, 896)
    logits = net(dummy_x)
    print(logits.size())